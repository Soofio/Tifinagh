...
/* Joue le fichier audio local de la lettre s'il existe, sinon bascule sur
   la synthèse vocale (approximative) et le signale visuellement 1s. */
function playLetter(letter, btnEl){
 const path=`audio/${letter.audio}.mp3`;
 ...
 audio.addEventListener("canplaythrough",()=>{if(!handled){handled=true;audio.play().catch(useFallback)}});
 audio.addEventListener("error",useFallback);
 setTimeout(useFallback,900); // filet de sécurité si aucun évènement ne se déclenche
 audio.load();
}
